...
export function useRelatorioCondominiosGeorreferenciados() {
  const gerarRelatorioCondominiosGeorreferenciados = async (filtros: FiltrosRelatorioType) => {
    let query = supabase
      .from('empreendimentos_terceirizados')
      .select('nome, uf, rota, quantidade_medidores, latitude, longitude')
      .order('uf')
      .order('nome');
  ...
  return { gerarRelatorioCondominiosGeorreferenciados };
}
