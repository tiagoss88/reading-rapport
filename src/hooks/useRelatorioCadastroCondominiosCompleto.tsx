...
export function useRelatorioCadastroCondominiosCompleto() {
  const gerarRelatorioCadastroCondominiosCompleto = async (filtros: FiltrosRelatorioType) => {
    let query = supabase
      .from('empreendimentos_terceirizados')
      .select('nome, uf, rota, quantidade_medidores, endereco')
      .order('uf')
      .order('nome');
  ...
  return { gerarRelatorioCadastroCondominiosCompleto };
}
