...
export interface ComprovantePagamentoData {
  ...
  data_hora?: string | null; // já formatada dd/MM/yyyy HH:mm
  cliente?: string | null;
...
function buildHTML(d: ComprovantePagamentoData) {
  const unidade =
    [d.bloco && `${d.bloco}`, d.apartamento && `${d.apartamento}`]
      .filter(Boolean)
      .join(' / ') || '—';

  const qrUrl =
    'https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=' +
    encodeURIComponent('https://www.agasen.com.br');
  ...
  return `
    <div class="agasen-receipt" style="
      ...
      <div style="text-align:center;margin-bottom:25px;">
        <img src="${logoUrl}" alt="Agasen" crossorigin="anonymous"
      ...
      <div style="border-top:1px dashed #26a9b5;border-bottom:1px dashed #26a9b5;
        padding:10px 0;margin:20px 0;text-align:center;">
        <h2 style="margin:0;font-size:14px;letter-spacing:2px;color:#003366;">COMPROVANTE DE PAGAMENTO</h2>
      </div>

      <div style="font-family:'JetBrains Mono',monospace;font-size:12px;margin-bottom:20px;">
      ...
      <div style="background:#e9f6f7;border:1px solid #26a9b5;padding:15px;
        display:flex;justify-content:space-between;align-items:center;margin-top:20px;border-radius:4px;">
      ...
      <div style="margin-top:30px;display:flex;flex-direction:column;align-items:center;gap:10px;
        padding-top:20px;border-top:1px dashed #26a9b5;">
        <div style="width:100px;height:100px;background:#fff;display:flex;align-items:center;
          justify-content:center;border:1px solid #26a9b5;padding:5px;box-sizing:border-box;">
          <img src="${qrUrl}" alt="QR" crossorigin="anonymous" style="width:100%;height:100%;display:block;" />
        </div>
...
function row(label: string, value: string) {
  return `<div style="display:flex;justify-content:space-between;margin-bottom:8px;
    border-bottom:1px dotted #eee;padding-bottom:4px;gap:8px;">
    <span style="color:#888;text-transform:uppercase;font-size:10px;">${label}</span>
